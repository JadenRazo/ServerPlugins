...
/** Command to coordinate server shutdown across the network. */
public class GShutdownCommand implements SimpleCommand {
    ...
    public void execute(Invocation invocation) {
        ...
        if (!sender.hasPermission("serveradmin.gshutdown")) {
            sender.sendMessage(VelocityTextUtil.parse("<red>You don't have permission!"));
            return;
        }

        // Usage: /gshutdown <server|all> [delay] [reason...]
        if (args.length < 1) {
            sender.sendMessage(
                    VelocityTextUtil.parse(
                            "<red>Usage: /gshutdown <server|all> [delay] [reason...]"));
            sender.sendMessage(
                    VelocityTextUtil.parse(
                            "<gray>Example: /gshutdown smp 30 Scheduled maintenance"));
            return;
        }
        ...
        redisClient.publishServerControl("SHUTDOWN", targetServer, delay, reason, staffName);

        // Confirm to sender
        sender.sendMessage(
                VelocityTextUtil.parse(
                        "<green>Shutdown signal sent to <white>"
                                + targetServer
                                + "</white> with <white>"
                                + delay
                                + "</white> second delay."));
        ...
        logger.info(
                "{} initiated shutdown of {} (delay: {}s, reason: {})",
                staffName,
                targetServer,
                delay,
                reason);
    ...
    public boolean hasPermission(Invocation invocation) {
        return invocation.source().hasPermission("serveradmin.gshutdown");
    }
}
